...
namespace TwoMGFX
{
    ...
    {
        public static ShaderData CreateGLSL(
            byte[] byteCode, bool isVertexShader, List<ConstantBufferData> cbuffers,
            int sharedIndex, Dictionary<string, SamplerStateInfo> samplerStates, bool debug)
        {
            ...
            var parseData = Marshal.PtrToStructure<MojoShader.MOJOSHADER_parseData>(parseDataPtr);
            ...
            Array.Sort(symbols, (MojoShader.MOJOSHADER_symbol a, MojoShader.MOJOSHADER_symbol b) =>
            {
            ...
            // For whatever reason the register indexing is incorrect from MojoShader.
            {
                uint bool_index = 0;
                uint float4_index = 0;
                uint int4_index = 0;
            ...
            for (var i = 0; i < samplers.Length; i++)
            {
                ...
                if (samplerStates.TryGetValue(originalSamplerName, out SamplerStateInfo state))
                {
            ...
            {
                var cbuffer = new ConstantBufferData(
                    symbol_types[i].name, symbol_types[i].set, symbols);
