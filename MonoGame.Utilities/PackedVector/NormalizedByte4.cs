...
namespace MonoGame.Utilities.PackedVector
{
    public struct NormalizedByte4 : IPackedVector<uint>, IEquatable<NormalizedByte4>
    {
        ...
        {
            var byte4 = (((uint) Math.Round(MathHelper.Clamp(x, -1f, 1f) * 127.0f)) & 0xff) << 0;
            var byte3 = (((uint) Math.Round(MathHelper.Clamp(y, -1f, 1f) * 127.0f)) & 0xff) << 8;
            var byte2 = (((uint) Math.Round(MathHelper.Clamp(z, -1f, 1f) * 127.0f)) & 0xff) << 16;
            var byte1 = (((uint) Math.Round(MathHelper.Clamp(w, -1f, 1f) * 127.0f)) & 0xff) << 24;
