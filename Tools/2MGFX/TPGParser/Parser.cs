...
namespace TwoMGFX.TPGParser
{
    ...
    public partial class Parser
    {
        ...
        private void ParseStart(ParseNode parent) // NonTerminalSymbol: Start
        {
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Code, TokenType.Technique, TokenType.Sampler); // ZeroOrMore Rule
            while (tok.Type == TokenType.Code
                || tok.Type == TokenType.Technique
                || tok.Type == TokenType.Sampler)
            {
                tok = _scanner.LookAhead(TokenType.Code, TokenType.Technique, TokenType.Sampler); // Choice Rule
                switch (tok.Type)
                { // Choice Rule
                    case TokenType.Code:
                        tok = _scanner.Scan(TokenType.Code); // Terminal Rule: Code
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.EndOfFile); // Terminal Rule: EndOfFile
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseTechnique_Declaration(ParseNode parent) // NonTerminalSymbol: Technique_Declaration
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Technique_Declaration), "Technique_Declaration");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.Technique); // Terminal Rule: Technique
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Identifier); // Option Rule
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.OpenBracket); // Terminal Rule: OpenBracket
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            { // OneOrMore Rule
                ...
                tok = _scanner.LookAhead(TokenType.Pass); // OneOrMore Rule
            } while (tok.Type == TokenType.Pass); // OneOrMore Rule

            // Concat Rule
            tok = _scanner.Scan(TokenType.CloseBracket); // Terminal Rule: CloseBracket
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseFillMode_Solid(ParseNode parent) // NonTerminalSymbol: FillMode_Solid
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.FillMode_Solid), "FillMode_Solid");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Solid); // Terminal Rule: Solid
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseFillMode_WireFrame(ParseNode parent) // NonTerminalSymbol: FillMode_WireFrame
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.FillMode_WireFrame), "FillMode_WireFrame");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.WireFrame); // Terminal Rule: WireFrame
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseFillModes(ParseNode parent) // NonTerminalSymbol: FillModes
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.FillModes), "FillModes");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Solid, TokenType.WireFrame); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
        ...
        private void ParseCullMode_None(ParseNode parent) // NonTerminalSymbol: CullMode_None
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CullMode_None), "CullMode_None");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.None); // Terminal Rule: None
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCullMode_Cw(ParseNode parent) // NonTerminalSymbol: CullMode_Cw
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CullMode_Cw), "CullMode_Cw");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Cw); // Terminal Rule: Cw
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCullMode_Ccw(ParseNode parent) // NonTerminalSymbol: CullMode_Ccw
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CullMode_Ccw), "CullMode_Ccw");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Ccw); // Terminal Rule: Ccw
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCullModes(ParseNode parent) // NonTerminalSymbol: CullModes
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CullModes), "CullModes");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.None, TokenType.Cw, TokenType.Ccw); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.None:
        ...
        private void ParseColors_None(ParseNode parent) // NonTerminalSymbol: Colors_None
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors_None), "Colors_None");
            parent.Nodes.Add(node);
        ...
        private void ParseColors_Red(ParseNode parent) // NonTerminalSymbol: Colors_Red
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors_Red), "Colors_Red");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Red); // Terminal Rule: Red
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseColors_Green(ParseNode parent) // NonTerminalSymbol: Colors_Green
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors_Green), "Colors_Green");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Green); // Terminal Rule: Green
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseColors_Blue(ParseNode parent) // NonTerminalSymbol: Colors_Blue
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors_Blue), "Colors_Blue");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Blue); // Terminal Rule: Blue
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseColors_Alpha(ParseNode parent) // NonTerminalSymbol: Colors_Alpha
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors_Alpha), "Colors_Alpha");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Alpha); // Terminal Rule: Alpha
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseColors_All(ParseNode parent) // NonTerminalSymbol: Colors_All
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors_All), "Colors_All");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.All); // Terminal Rule: All
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseColors_Boolean(ParseNode parent) // NonTerminalSymbol: Colors_Boolean
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors_Boolean), "Colors_Boolean");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Boolean); // Terminal Rule: Boolean
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseColors(ParseNode parent) // NonTerminalSymbol: Colors
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Colors), "Colors");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Red, TokenType.Green, TokenType.Blue, TokenType.Alpha, TokenType.None, TokenType.All, TokenType.Boolean); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.Red:
        ...
        private void ParseColorsMasks(ParseNode parent) // NonTerminalSymbol: ColorsMasks
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.ColorsMasks), "ColorsMasks");
            parent.Nodes.Add(node);
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Or); // Option Rule
        ...
        private void ParseBlend_Zero(ParseNode parent) // NonTerminalSymbol: Blend_Zero
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_Zero), "Blend_Zero");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Zero); // Terminal Rule: Zero
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_One(ParseNode parent) // NonTerminalSymbol: Blend_One
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_One), "Blend_One");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.One); // Terminal Rule: One
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_SrcColor(ParseNode parent) // NonTerminalSymbol: Blend_SrcColor
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_SrcColor), "Blend_SrcColor");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.SrcColor); // Terminal Rule: SrcColor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_InvSrcColor(ParseNode parent) // NonTerminalSymbol: Blend_InvSrcColor
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_InvSrcColor), "Blend_InvSrcColor");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.InvSrcColor); // Terminal Rule: InvSrcColor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_SrcAlpha(ParseNode parent) // NonTerminalSymbol: Blend_SrcAlpha
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_SrcAlpha), "Blend_SrcAlpha");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.SrcAlpha); // Terminal Rule: SrcAlpha
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_InvSrcAlpha(ParseNode parent) // NonTerminalSymbol: Blend_InvSrcAlpha
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_InvSrcAlpha), "Blend_InvSrcAlpha");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.InvSrcAlpha); // Terminal Rule: InvSrcAlpha
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_DestAlpha(ParseNode parent) // NonTerminalSymbol: Blend_DestAlpha
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_DestAlpha), "Blend_DestAlpha");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.DestAlpha); // Terminal Rule: DestAlpha
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_InvDestAlpha(ParseNode parent) // NonTerminalSymbol: Blend_InvDestAlpha
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_InvDestAlpha), "Blend_InvDestAlpha");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.InvDestAlpha); // Terminal Rule: InvDestAlpha
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_DestColor(ParseNode parent) // NonTerminalSymbol: Blend_DestColor
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_DestColor), "Blend_DestColor");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.DestColor); // Terminal Rule: DestColor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_InvDestColor(ParseNode parent) // NonTerminalSymbol: Blend_InvDestColor
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_InvDestColor), "Blend_InvDestColor");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.InvDestColor); // Terminal Rule: InvDestColor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_SrcAlphaSat(ParseNode parent) // NonTerminalSymbol: Blend_SrcAlphaSat
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_SrcAlphaSat), "Blend_SrcAlphaSat");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.SrcAlphaSat); // Terminal Rule: SrcAlphaSat
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_BlendFactor(ParseNode parent) // NonTerminalSymbol: Blend_BlendFactor
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_BlendFactor), "Blend_BlendFactor");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.BlendFactor); // Terminal Rule: BlendFactor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlend_InvBlendFactor(ParseNode parent) // NonTerminalSymbol: Blend_InvBlendFactor
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blend_InvBlendFactor), "Blend_InvBlendFactor");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.InvBlendFactor); // Terminal Rule: InvBlendFactor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlends(ParseNode parent) // NonTerminalSymbol: Blends
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Blends), "Blends");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Zero, TokenType.One, TokenType.SrcColor, TokenType.InvSrcColor, TokenType.SrcAlpha, TokenType.InvSrcAlpha, TokenType.DestAlpha, TokenType.InvDestAlpha, TokenType.DestColor, TokenType.InvDestColor, TokenType.SrcAlphaSat, TokenType.BlendFactor, TokenType.InvBlendFactor); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.Zero:
        ...
        private void ParseBlendOp_Add(ParseNode parent) // NonTerminalSymbol: BlendOp_Add
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.BlendOp_Add), "BlendOp_Add");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Add); // Terminal Rule: Add
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlendOp_Subtract(ParseNode parent) // NonTerminalSymbol: BlendOp_Subtract
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.BlendOp_Subtract), "BlendOp_Subtract");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Subtract); // Terminal Rule: Subtract
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlendOp_RevSubtract(ParseNode parent) // NonTerminalSymbol: BlendOp_RevSubtract
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.BlendOp_RevSubtract), "BlendOp_RevSubtract");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.RevSubtract); // Terminal Rule: RevSubtract
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlendOp_Min(ParseNode parent) // NonTerminalSymbol: BlendOp_Min
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.BlendOp_Min), "BlendOp_Min");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Min); // Terminal Rule: Min
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlendOp_Max(ParseNode parent) // NonTerminalSymbol: BlendOp_Max
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.BlendOp_Max), "BlendOp_Max");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Max); // Terminal Rule: Max
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseBlendOps(ParseNode parent) // NonTerminalSymbol: BlendOps
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.BlendOps), "BlendOps");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Add, TokenType.Subtract, TokenType.RevSubtract, TokenType.Min, TokenType.Max); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.Add:
        ...
        private void ParseCmpFunc_Never(ParseNode parent) // NonTerminalSymbol: CmpFunc_Never
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_Never), "CmpFunc_Never");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Never); // Terminal Rule: Never
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc_Less(ParseNode parent) // NonTerminalSymbol: CmpFunc_Less
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_Less), "CmpFunc_Less");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Less); // Terminal Rule: Less
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc_Equal(ParseNode parent) // NonTerminalSymbol: CmpFunc_Equal
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_Equal), "CmpFunc_Equal");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Equal); // Terminal Rule: Equal
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc_LessEqual(ParseNode parent) // NonTerminalSymbol: CmpFunc_LessEqual
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_LessEqual), "CmpFunc_LessEqual");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.LessEqual); // Terminal Rule: LessEqual
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc_Greater(ParseNode parent) // NonTerminalSymbol: CmpFunc_Greater
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_Greater), "CmpFunc_Greater");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Greater); // Terminal Rule: Greater
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc_NotEqual(ParseNode parent) // NonTerminalSymbol: CmpFunc_NotEqual
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_NotEqual), "CmpFunc_NotEqual");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.NotEqual); // Terminal Rule: NotEqual
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc_GreaterEqual(ParseNode parent) // NonTerminalSymbol: CmpFunc_GreaterEqual
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_GreaterEqual), "CmpFunc_GreaterEqual");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.GreaterEqual); // Terminal Rule: GreaterEqual
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc_Always(ParseNode parent) // NonTerminalSymbol: CmpFunc_Always
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc_Always), "CmpFunc_Always");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Always); // Terminal Rule: Always
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseCmpFunc(ParseNode parent) // NonTerminalSymbol: CmpFunc
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.CmpFunc), "CmpFunc");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Never, TokenType.Less, TokenType.Equal, TokenType.LessEqual, TokenType.Greater, TokenType.NotEqual, TokenType.GreaterEqual, TokenType.Always); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
        ...
        private void ParseStencilOp_Keep(ParseNode parent) // NonTerminalSymbol: StencilOp_Keep
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_Keep), "StencilOp_Keep");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Keep); // Terminal Rule: Keep
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseStencilOp_Zero(ParseNode parent) // NonTerminalSymbol: StencilOp_Zero
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_Zero), "StencilOp_Zero");
            parent.Nodes.Add(node);
        ...
        private void ParseStencilOp_Replace(ParseNode parent) // NonTerminalSymbol: StencilOp_Replace
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_Replace), "StencilOp_Replace");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Replace); // Terminal Rule: Replace
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseStencilOp_IncrSat(ParseNode parent) // NonTerminalSymbol: StencilOp_IncrSat
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_IncrSat), "StencilOp_IncrSat");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.IncrSat); // Terminal Rule: IncrSat
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseStencilOp_DecrSat(ParseNode parent) // NonTerminalSymbol: StencilOp_DecrSat
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_DecrSat), "StencilOp_DecrSat");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.DecrSat); // Terminal Rule: DecrSat
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseStencilOp_Invert(ParseNode parent) // NonTerminalSymbol: StencilOp_Invert
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_Invert), "StencilOp_Invert");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Invert); // Terminal Rule: Invert
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseStencilOp_Incr(ParseNode parent) // NonTerminalSymbol: StencilOp_Incr
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_Incr), "StencilOp_Incr");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Incr); // Terminal Rule: Incr
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseStencilOp_Decr(ParseNode parent) // NonTerminalSymbol: StencilOp_Decr
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp_Decr), "StencilOp_Decr");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Decr); // Terminal Rule: Decr
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseStencilOp(ParseNode parent) // NonTerminalSymbol: StencilOp
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.StencilOp), "StencilOp");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Keep, TokenType.Zero, TokenType.Replace, TokenType.IncrSat, TokenType.DecrSat, TokenType.Invert, TokenType.Incr, TokenType.Decr); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.Keep:
        ...
        private void ParseRender_State_CullMode(ParseNode parent) // NonTerminalSymbol: Render_State_CullMode
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_CullMode), "Render_State_CullMode");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.CullMode); // Terminal Rule: CullMode
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.Equals); // Terminal Rule: Equals
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.Semicolon); // Terminal Rule: Semicolon
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_FillMode(ParseNode parent) // NonTerminalSymbol: Render_State_FillMode
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_FillMode), "Render_State_FillMode");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.FillMode); // Terminal Rule: FillMode
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_AlphaBlendEnable(ParseNode parent) // NonTerminalSymbol: Render_State_AlphaBlendEnable
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_AlphaBlendEnable), "Render_State_AlphaBlendEnable");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.AlphaBlendEnable); // Terminal Rule: AlphaBlendEnable
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_SrcBlend(ParseNode parent) // NonTerminalSymbol: Render_State_SrcBlend
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_SrcBlend), "Render_State_SrcBlend");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.SrcBlend); // Terminal Rule: SrcBlend
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_DestBlend(ParseNode parent) // NonTerminalSymbol: Render_State_DestBlend
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_DestBlend), "Render_State_DestBlend");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.DestBlend); // Terminal Rule: DestBlend
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_BlendOp(ParseNode parent) // NonTerminalSymbol: Render_State_BlendOp
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_BlendOp), "Render_State_BlendOp");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.BlendOp); // Terminal Rule: BlendOp
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_ColorWriteEnable(ParseNode parent) // NonTerminalSymbol: Render_State_ColorWriteEnable
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_ColorWriteEnable), "Render_State_ColorWriteEnable");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.ColorWriteEnable); // Terminal Rule: ColorWriteEnable
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_DepthBias(ParseNode parent) // NonTerminalSymbol: Render_State_DepthBias
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_DepthBias), "Render_State_DepthBias");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.DepthBias); // Terminal Rule: DepthBias
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.Number); // Terminal Rule: Number
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_SlopeScaleDepthBias(ParseNode parent) // NonTerminalSymbol: Render_State_SlopeScaleDepthBias
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_SlopeScaleDepthBias), "Render_State_SlopeScaleDepthBias");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.SlopeScaleDepthBias); // Terminal Rule: SlopeScaleDepthBias
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_ZEnable(ParseNode parent) // NonTerminalSymbol: Render_State_ZEnable
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_ZEnable), "Render_State_ZEnable");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.ZEnable); // Terminal Rule: ZEnable
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_ZWriteEnable(ParseNode parent) // NonTerminalSymbol: Render_State_ZWriteEnable
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_ZWriteEnable), "Render_State_ZWriteEnable");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.ZWriteEnable); // Terminal Rule: ZWriteEnable
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_ZFunc(ParseNode parent) // NonTerminalSymbol: Render_State_ZFunc
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_ZFunc), "Render_State_ZFunc");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.ZFunc); // Terminal Rule: ZFunc
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_MultiSampleAntiAlias(ParseNode parent) // NonTerminalSymbol: Render_State_MultiSampleAntiAlias
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_MultiSampleAntiAlias), "Render_State_MultiSampleAntiAlias");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.MultiSampleAntiAlias); // Terminal Rule: MultiSampleAntiAlias
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_ScissorTestEnable(ParseNode parent) // NonTerminalSymbol: Render_State_ScissorTestEnable
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_ScissorTestEnable), "Render_State_ScissorTestEnable");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.ScissorTestEnable); // Terminal Rule: ScissorTestEnable
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilEnable(ParseNode parent) // NonTerminalSymbol: Render_State_StencilEnable
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilEnable), "Render_State_StencilEnable");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilEnable); // Terminal Rule: StencilEnable
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilFail(ParseNode parent) // NonTerminalSymbol: Render_State_StencilFail
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilFail), "Render_State_StencilFail");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilFail); // Terminal Rule: StencilFail
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilFunc(ParseNode parent) // NonTerminalSymbol: Render_State_StencilFunc
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilFunc), "Render_State_StencilFunc");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilFunc); // Terminal Rule: StencilFunc
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilMask(ParseNode parent) // NonTerminalSymbol: Render_State_StencilMask
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilMask), "Render_State_StencilMask");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilMask); // Terminal Rule: StencilMask
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilPass(ParseNode parent) // NonTerminalSymbol: Render_State_StencilPass
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilPass), "Render_State_StencilPass");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilPass); // Terminal Rule: StencilPass
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilRef(ParseNode parent) // NonTerminalSymbol: Render_State_StencilRef
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilRef), "Render_State_StencilRef");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilRef); // Terminal Rule: StencilRef
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilWriteMask(ParseNode parent) // NonTerminalSymbol: Render_State_StencilWriteMask
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilWriteMask), "Render_State_StencilWriteMask");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilWriteMask); // Terminal Rule: StencilWriteMask
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseRender_State_StencilZFail(ParseNode parent) // NonTerminalSymbol: Render_State_StencilZFail
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Render_State_StencilZFail), "Render_State_StencilZFail");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.StencilZFail); // Terminal Rule: StencilZFail
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParsePass_Declaration(ParseNode parent) // NonTerminalSymbol: Pass_Declaration
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Pass_Declaration), "Pass_Declaration");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.Pass); // Terminal Rule: Pass
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseVertexShader_Pass_Expression(ParseNode parent) // NonTerminalSymbol: VertexShader_Pass_Expression
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.VertexShader_Pass_Expression), "VertexShader_Pass_Expression");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.VertexShader); // Terminal Rule: VertexShader
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.Compile); // Terminal Rule: Compile
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.ShaderModel); // Terminal Rule: ShaderModel
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.OpenParenthesis); // Terminal Rule: OpenParenthesis
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.CloseParenthesis); // Terminal Rule: CloseParenthesis
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParsePixelShader_Pass_Expression(ParseNode parent) // NonTerminalSymbol: PixelShader_Pass_Expression
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.PixelShader_Pass_Expression), "PixelShader_Pass_Expression");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.PixelShader); // Terminal Rule: PixelShader
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseAddressMode_Clamp(ParseNode parent) // NonTerminalSymbol: AddressMode_Clamp
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.AddressMode_Clamp), "AddressMode_Clamp");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Clamp); // Terminal Rule: Clamp
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseAddressMode_Wrap(ParseNode parent) // NonTerminalSymbol: AddressMode_Wrap
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.AddressMode_Wrap), "AddressMode_Wrap");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Wrap); // Terminal Rule: Wrap
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseAddressMode_Mirror(ParseNode parent) // NonTerminalSymbol: AddressMode_Mirror
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.AddressMode_Mirror), "AddressMode_Mirror");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Mirror); // Terminal Rule: Mirror
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseAddressMode_Border(ParseNode parent) // NonTerminalSymbol: AddressMode_Border
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.AddressMode_Border), "AddressMode_Border");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Border); // Terminal Rule: Border
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseAddressMode(ParseNode parent) // NonTerminalSymbol: AddressMode
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.AddressMode), "AddressMode");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Clamp, TokenType.Wrap, TokenType.Mirror, TokenType.Border); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
        ...
        private void ParseTextureFilter_None(ParseNode parent) // NonTerminalSymbol: TextureFilter_None
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.TextureFilter_None), "TextureFilter_None");
            parent.Nodes.Add(node);
        ...
        private void ParseTextureFilter_Linear(ParseNode parent) // NonTerminalSymbol: TextureFilter_Linear
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.TextureFilter_Linear), "TextureFilter_Linear");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Linear); // Terminal Rule: Linear
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseTextureFilter_Point(ParseNode parent) // NonTerminalSymbol: TextureFilter_Point
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.TextureFilter_Point), "TextureFilter_Point");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Point); // Terminal Rule: Point
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseTextureFilter_Anisotropic(ParseNode parent) // NonTerminalSymbol: TextureFilter_Anisotropic
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.TextureFilter_Anisotropic), "TextureFilter_Anisotropic");
            parent.Nodes.Add(node);

            tok = _scanner.Scan(TokenType.Anisotropic); // Terminal Rule: Anisotropic
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseTextureFilter(ParseNode parent) // NonTerminalSymbol: TextureFilter
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.TextureFilter), "TextureFilter");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.None, TokenType.Linear, TokenType.Point, TokenType.Anisotropic); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.None:
        ...
        private void ParseSampler_State_Texture(ParseNode parent) // NonTerminalSymbol: Sampler_State_Texture
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_Texture), "Sampler_State_Texture");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.Texture); // Terminal Rule: Texture
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.LessThan, TokenType.OpenParenthesis); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.LessThan:
                    tok = _scanner.Scan(TokenType.LessThan); // Terminal Rule: LessThan
                    n = node.CreateNode(tok, tok.ToString());
                    node.Token.UpdateRange(tok);
                    node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.GreaterThan, TokenType.CloseParenthesis); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
                case TokenType.GreaterThan:
                    tok = _scanner.Scan(TokenType.GreaterThan); // Terminal Rule: GreaterThan
                    n = node.CreateNode(tok, tok.ToString());
                    node.Token.UpdateRange(tok);
                    node.Nodes.Add(n);
        ...
        private void ParseSampler_State_MinFilter(ParseNode parent) // NonTerminalSymbol: Sampler_State_MinFilter
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_MinFilter), "Sampler_State_MinFilter");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.MinFilter); // Terminal Rule: MinFilter
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_MagFilter(ParseNode parent) // NonTerminalSymbol: Sampler_State_MagFilter
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_MagFilter), "Sampler_State_MagFilter");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.MagFilter); // Terminal Rule: MagFilter
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_MipFilter(ParseNode parent) // NonTerminalSymbol: Sampler_State_MipFilter
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_MipFilter), "Sampler_State_MipFilter");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.MipFilter); // Terminal Rule: MipFilter
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_Filter(ParseNode parent) // NonTerminalSymbol: Sampler_State_Filter
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_Filter), "Sampler_State_Filter");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.Filter); // Terminal Rule: Filter
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_AddressU(ParseNode parent) // NonTerminalSymbol: Sampler_State_AddressU
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_AddressU), "Sampler_State_AddressU");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.AddressU); // Terminal Rule: AddressU
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_AddressV(ParseNode parent) // NonTerminalSymbol: Sampler_State_AddressV
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_AddressV), "Sampler_State_AddressV");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.AddressV); // Terminal Rule: AddressV
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_AddressW(ParseNode parent) // NonTerminalSymbol: Sampler_State_AddressW
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_AddressW), "Sampler_State_AddressW");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.AddressW); // Terminal Rule: AddressW
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_BorderColor(ParseNode parent) // NonTerminalSymbol: Sampler_State_BorderColor
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_BorderColor), "Sampler_State_BorderColor");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.BorderColor); // Terminal Rule: BorderColor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.HexColor); // Terminal Rule: HexColor
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_MaxMipLevel(ParseNode parent) // NonTerminalSymbol: Sampler_State_MaxMipLevel
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_MaxMipLevel), "Sampler_State_MaxMipLevel");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.MaxMipLevel); // Terminal Rule: MaxMipLevel
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_MaxAnisotropy(ParseNode parent) // NonTerminalSymbol: Sampler_State_MaxAnisotropy
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_MaxAnisotropy), "Sampler_State_MaxAnisotropy");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.MaxAnisotropy); // Terminal Rule: MaxAnisotropy
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_MipLodBias(ParseNode parent) // NonTerminalSymbol: Sampler_State_MipLodBias
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_MipLodBias), "Sampler_State_MipLodBias");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.MipLodBias); // Terminal Rule: MipLodBias
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
        ...
        private void ParseSampler_State_Expression(ParseNode parent) // NonTerminalSymbol: Sampler_State_Expression
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_State_Expression), "Sampler_State_Expression");
            parent.Nodes.Add(node);

            tok = _scanner.LookAhead(TokenType.Texture, TokenType.MinFilter, TokenType.MagFilter, TokenType.MipFilter, TokenType.Filter, TokenType.AddressU, TokenType.AddressV, TokenType.AddressW, TokenType.BorderColor, TokenType.MaxMipLevel, TokenType.MaxAnisotropy, TokenType.MipLodBias); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
        ...
        private void ParseSampler_Register_Expression(ParseNode parent) // NonTerminalSymbol: Sampler_Register_Expression
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_Register_Expression), "Sampler_Register_Expression");
            parent.Nodes.Add(node);
            ...
            // Concat Rule
            tok = _scanner.Scan(TokenType.Register); // Terminal Rule: Register
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Comma); // Option Rule
            if (tok.Type == TokenType.Comma)
            {
                ...
                // Concat Rule
                tok = _scanner.LookAhead(TokenType.OpenSquareBracket); // Option Rule
                if (tok.Type == TokenType.OpenSquareBracket)
                {

                    // Concat Rule
                    tok = _scanner.Scan(TokenType.OpenSquareBracket); // Terminal Rule: OpenSquareBracket
                    n = node.CreateNode(tok, tok.ToString());
                    node.Token.UpdateRange(tok);
                    node.Nodes.Add(n);
                    if (tok.Type != TokenType.OpenSquareBracket)
                    ...
                    // Concat Rule
                    tok = _scanner.Scan(TokenType.CloseSquareBracket); // Terminal Rule: CloseSquareBracket
                    n = node.CreateNode(tok, tok.ToString());
                    node.Token.UpdateRange(tok);
                    node.Nodes.Add(n);
        ...
        private void ParseSampler_Declaration_States(ParseNode parent) // NonTerminalSymbol: Sampler_Declaration_States
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_Declaration_States), "Sampler_Declaration_States");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Equals); // Option Rule
            if (tok.Type == TokenType.Equals)
            {
                ...
                // Concat Rule
                tok = _scanner.Scan(TokenType.SamplerState); // Terminal Rule: SamplerState
                n = node.CreateNode(tok, tok.ToString());
                node.Token.UpdateRange(tok);
                node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Texture, TokenType.MinFilter, TokenType.MagFilter, TokenType.MipFilter, TokenType.Filter, TokenType.AddressU, TokenType.AddressV, TokenType.AddressW, TokenType.BorderColor, TokenType.MaxMipLevel, TokenType.MaxAnisotropy, TokenType.MipLodBias); // ZeroOrMore Rule
        ...
        private void ParseSampler_Declaration(ParseNode parent) // NonTerminalSymbol: Sampler_Declaration
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(_scanner.GetToken(TokenType.Sampler_Declaration), "Sampler_Declaration");
            parent.Nodes.Add(node);


            // Concat Rule
            tok = _scanner.Scan(TokenType.Sampler); // Terminal Rule: Sampler
            n = node.CreateNode(tok, tok.ToString());
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Colon); // ZeroOrMore Rule
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Equals, TokenType.OpenBracket); // Option Rule
            if (tok.Type == TokenType.Equals
            ...
            // Concat Rule
            tok = _scanner.LookAhead(TokenType.Semicolon, TokenType.Comma, TokenType.CloseParenthesis); // Choice Rule
            switch (tok.Type)
            { // Choice Rule
