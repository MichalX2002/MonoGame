...
namespace MonoGame.Content.Builder
{
    ...
    public class MGBuildParser
    {
        ...
        public bool Parse(IEnumerable<string> args)
        {
            ...
            var missingRequiredOption = _requiredOptions.FirstOrDefault(
                field => !IsList(field) || GetList(field).Count == 0);
            
            if (missingRequiredOption != null)
            {
                ShowError(
                    "Missing argument '{0}'",
                    GetAttribute<CommandLineParameterAttribute>(missingRequiredOption).Name);
                return false;
            }
        ...
        private void ParsePreprocessArg(
            string arg, List<string> output, Stack<IfCondition> ifstack, bool inResponseFile)
        {
        ...
        bool SetOption(MemberInfo member, string value)
        {
                ...
                else
                {
                    ...
                    if (member is MethodInfo)
                    {
                        ...
                        if (parameters.Length == 0)
                            ...
                            method.Invoke(
                                _optionsObject, new[] { ChangeType(value, parameters[0].ParameterType) });
                    }
            ...
            catch
            {
                ShowError(
                    ...
                    value,
                    GetAttribute<CommandLineParameterAttribute>(member).Name);
                return false;
            }
        }
            ...
            foreach (var i in ReservedPrefixes)
            {
                if (str.StartsWith(i))
                    throw new Exception(string.Format(
                        "'{0}' is a reserved prefix and cannot be used at the start of an argument name.", i));
            }
        }
        ...
        static Type ListElementType(MemberInfo member)
        {
            if (member is FieldInfo)
            {
                var field = member as FieldInfo;
                var interfaces = 
                    from i in field.FieldType.GetInterfaces()
