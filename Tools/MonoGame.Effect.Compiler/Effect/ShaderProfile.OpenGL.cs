...
namespace MonoGame.Effect
{
    ...
    {
        private static readonly Regex GlslPixelShaderRegex = new Regex(
            @"^ps_(?<major>1|2|3|4|5)_(?<minor>0|1|)$", RegexOptions.Compiled);

        private static readonly Regex GlslVertexShaderRegex = new Regex(
            @"^vs_(?<major>1|2|3|4|5)_(?<minor>0|1|)$", RegexOptions.Compiled);

        public OpenGLShaderProfile() : base("OpenGL", 0)
        {
        }
            ...
            bool isVertexShader, EffectObject effect, ref string errorsAndWarnings)
        {
            ...
            var shaderData = ShaderData.CreateGLSL(
